...
def main()->int:
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--formal-benchmark-dir',type=Path,required=True);p.add_argument('--component-audit-dir',type=Path,required=True);p.add_argument('--output-dir',type=Path,required=True);a=p.parse_args()
    if a.output_dir.exists():raise RuntimeError(f'refusing_existing_output_directory:{a.output_dir}')
    truth=_read(a.formal_benchmark_dir/'truth_table.csv');components=_read(a.component_audit_dir/'component_truth_table.csv');splits=_read(a.formal_benchmark_dir/'state_splits.csv')
    ...
    for name in sorted({str(r['model']) for r in outputs}):
      for slice_name in sorted({str(r['slice']) for r in outputs}):
       for seed in sorted({int(r['seed']) for r in outputs if r['model']==name}):
        ...
        labels=np.asarray([bool(r['hard_logic_feasible']) and r['true_cost']<r['training_threshold'] for r in rows]);candidate=np.asarray([r['prediction']<r['training_threshold'] if r['kind']=='cost' else r['prediction']>0 for r in rows]);classification.append({'model':name,'seed':seed,'slice':slice_name,**_class(labels,candidate)})
    ...
    protocol={'version':'stage2-controlled-alternatives-v2','truth_source':'formal benchmark truth_table only','hard_logic_source':'supplemental component audit joined by formal key','edlp_calls':0,'residual_labels':'fit-only true_cost - fit-only quadratic prediction','qnn_residual':{'layers':1,'maxiter':20,'seeds':list(SEEDS)},'margin_tau':'training_indices minimum true cost at same load','feasible_positive':'hard_logic_feasible and true_cost strictly below training incumbent','no_test_selection':True}
